...
def binary_search_eigenvalue(function,q,B,mass,Cross,m,n, low, high, tolerance, target):
    ...
    while (high - low) / 2.0 > tolerance:
        mid= (low + high) / 2.0
        if function(mid,q,B,mass,Cross,m,n) < target:
            high = mid
        else:
            low = mid
    return (low + high) / 2.0
...
if __name__ == "__main__":
    ...
    # 一次性写入所有数据到 CSV 文件
    with open('q_nonzero_Tc_data2.csv', 'w', newline='') as csvfile:
    ...
    plt.savefig('q_nonzero_Tc_data2.svg', format='svg')
    plt.show()

    filename = 'B-Tc-nonzero2.csv'
    ...
    plt.grid(True)
    plt.savefig('B-Tc-nonzero2.svg', format='svg')
    plt.show()
    end_time = time.time()
